...
@app.get("/tasks/{owner_id}", response_model=schemas.Task)
def read_task_by_owner_id(owner_id: int, db: Session = Depends(get_db)):
    result = crud.get_task_by_id(db, owner_id=owner_id)
    if result is None:
        raise HTTPException(status_code=404, detail="Task not found")
    return result
